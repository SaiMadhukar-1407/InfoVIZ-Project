...
const Dashboard = ({filePath, selectedData, selectedDate, onDateChange, availableDates, onFileUpload }) => {
  ...
  return (
    <div className="dashboard">
      ...
      {["CO(GT)", "C6H6(GT)", "NMHC(GT)", "NO2(GT)", "PT08.S5(O3)", "NOx(GT)"].map((pollutant) => (
        <div
          key={pollutant}
          className="metric"
          ...
          onClick={() => {
            ...
            const color = pollutantColors[pollutant] || "#000";
            navigate("/pollutant", { state: { pollutant, filePath , color } });
          }}
        >
          <strong>{pollutant.replace("(GT)", "")}</strong>
          <p>
            {selectedData ? selectedData[pollutant] : ""}{" "}
            {pollutant.includes("O3") ? "µg/m³" : "mg/m³"}
          </p>
        </div>
      ))}
    </div>
